...
const DEFAULT_TOPIC = 'cisco/iotorchestrator/v1/#';
...
function createMqttClient(instance, url, clientOptions, topic) {
  ...
  client.on('connect', () => {
    client.subscribe(topic, (err) => {
      if(!err) {
        console.log('barnowl-cisco: connected to MQTT server and subscribed');
      }
    });
  });
...
function handleMessage(message, topic, decoder, url, decodingOptions) {
  decoder.handleProtobuf(message, url, Date.now(), decodingOptions);
}
