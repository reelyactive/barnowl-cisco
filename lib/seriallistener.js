...
const AUTO_MANUFACTURER = 'Cisco Systems'; // TODO: check default name
...
class SerialListener {
  ...
  constructor(options) {
    ...
    openSerialPort(path, (err, serialPort, path) => {
      if(err) {
        return console.log('barnowl-cisco: error opening serial port',
                           err.message);
      }
...
function handleSerialEvents(instance) {
  ...
  instance.serialPort.on('close', () => {
    console.log('barnowl-cisco: serial port closed');
  });
  instance.serialPort.on('error', (err) => {
    console.log('barnowl-cisco: serial port error', err.message);
  });
}
      ...
      ports.forEach(port => {
        if(port.manufacturer === AUTO_MANUFACTURER) {
          serialPort = new SerialPort({ path: port.path, baudRate: BAUDRATE },
                                      (err) => {
            console.log('barnowl-cisco: auto serial path: \"' + port.path +
                        '\" was selected');
            return callback(err, serialPort, port.path);
          });
        }
        else if(port.manufacturer) {
          console.log('barnowl-cisco: alternate serial path: \"' +
